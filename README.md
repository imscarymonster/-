...
## 2. 核心功能
- 乘客端：利用实时地理位置与数据流，为乘客提供候车时精准的车辆到站动态倒计时与乘车时精准的车辆到达下一站的动态倒计时。
- 司机端：基于瞬时客流热力分布，在极短时间窗口内自动计算出最优车辆调度方案并直接反馈给摆渡车司机（不需要摆渡车司机主动请求），实现运力在不同线路间的智能“秒级”转线与柔性互帮。
- 管理员端：给管理员提供全园区各线路的瞬时客流热力分布、各摆渡车的实时位置及在途运力重构状态；在后台对车辆在线状态、司机排班及行驶日志进行管理；调阅并分析全网乘客平均候车时间、车辆无效空驶里程等核心效能统计指标。
